...
export class GameSession {
  ...
  /** Тот же самый детерминированный RNG (см. `rng` выше), открытый наружу ДЛЯ РЕШЕНИЙ БОТА
   * (`bot.ts`) — по прямому запросу, живой баг-репорт: «прогнать иной алгоритм стройки юнитов и
   * переиграть партию заново». Бот — такой же источник игровых случайностей, как катастрофы или рост
   * леса, и на него распространяется то же требование: случайность бота обязана идти через seed
   * партии, иначе повторный прогон ОДНОГО И ТОГО ЖЕ сохранения с тем же зерном расходится (у бота
   * случайны, в частности, порядок перебора регионов при расстановке стартовых жетонов и выбор цели
   * дипломатических резолюций — то есть расхождение начинается с самого первого хода и дальше только
   * копится). */
  botRandom(): number {
    return this.rng();
  }
  ...
  /** Сколько буфера осады у города осталось ПРЯМО СЕЙЧАС — чистое чтение для планирования (`bot.ts`:
   * концентрация огня), в отличие от `citySiegeDefense` НЕ заводит буфер побочным эффектом, если по
   * городу в этом цикле ещё не били. Нужно, чтобы бот мог заранее посчитать, хватит ли совместного
   * удара нескольких юнитов пробить гарнизон В ЭТОМ ЖЕ цикле (буфер копится только внутри цикла и
   * сбрасывается на его обороте — растянутая на несколько циклов осада не копится вовсе). */
  citySiegeRemaining(city: City): number {
    return this.citySiegeBuffer.get(city.id) ?? this.cityGarrisonDefense(city);
  }
