...
export const BUILDINGS: BuildingDef[] = [
  // ⚔ Военные
  B("kazarma", "Казарма", "military", "Бронзовое дело", 1, "Без действия — обычная цена юнита по эпохе, без карты — строит 1 Воина в любом своём городе", "1 Силикаты + 3 Лес", [specific("silicates", 1), specific("wood", 3)]),
  ...
  B("yadernyi_arsenal", "Ядерный арсенал", "military", "Атомная энергия", 5, "Без действия — 2 Уран + 1 Металл, без денег, без лимита цикла — +1 ядерное оружие в запас (предмет пока без применения — нет боя/системы целей)", "2 Силикаты + 1 Металл + 1 Уран", [specific("silicates", 2), specific("metalOre", 1), specific("uranium", 1)]),
  B("aeroport", "Аэропорт", "military", "Авиация", 5, "Без действия, без денег, без лимита цикла — переброска 1 своего юнита со столицы на любую клетку карты (сектор нейтрален/свой, на клетке нет вражеского юнита; чужой сектор без «Открытых границ» — блокируется)", "1 Силикаты + 2 Металл + 1 Углеводороды", [specific("silicates", 1), specific("metalOre", 2), specific("hydrocarbons", 1)]),

  // 💰 Экономические
  B("sklad", "Склад", "economy", "Стандартизация", 2, "Удваивает вместимость хранилища (6 → 12 единиц). Клик по своему Складу — как карта «Рабочий», но без неё: собрать регион на склад без действия + 1 💰 за каждую добытую единицу, доступно каждый цикл", "1 Силикаты + 1 Лес + 2 разных торговых ресурса", [specific("silicates", 1), specific("wood", 1), category("trade", 2)]),
  B("radiovyshka", "Радиовышка", "economy", "Радио", 5, "Без действия — 1💰, не больше 1 раза за цикл (требует 1 Углеводороды или 1 Электричество со склада) — +3 Контента: новый building-only торговый ресурс (кино/музыка и подобное), монополия владельца, как Промтовары у Фабрики", "1 Силикаты + 1 Металл + 2 разных торговых ресурса", [specific("silicates", 1), specific("metalOre", 1), category("trade", 2)], false, { resource: "content", qty: 3 }),
  B("rynok", "Рынок", "economy", "Экономика", 3, "Без действия — 1 Углеводороды или 1 Электричество со склада — тот же доход, что у карты «Торговец»: выберите свой город, доход по всей его торговой сети", "1 Силикаты + 3 разных торговых ресурса", [specific("silicates", 1), category("trade", 3)]),
  B("fabrika", "Фабрика", "economy", "Конвейер", 5, "Клик по своей Фабрике — 1 💰, без действия (требует 1 Углеводороды или 1 Электричество со склада), получить 3 Промтовара. Промтовары — торговый ресурс, которого нет на карте: единственный источник — Фабрика, то есть монополия её владельца", "1 Силикаты + 3 Металл", [specific("silicates", 1), specific("metalOre", 3)], false, { resource: "promtovary", qty: 3 }),

  // ⚙ Промышленные
  B("ges", "ГЭС", "industry", "Паровой двигатель", 4, "Клик по своей ГЭС — 3 💰, без действия, получить 1 Электричество", "1 Силикаты + 1 Лес + 2 Металл", [specific("silicates", 1), specific("wood", 1), specific("metalOre", 2)], false, { resource: "electricity", qty: 1 }),
  B("aes", "АЭС", "industry", "Атомная энергия", 5, "Клик по своей АЭС — 5 💰, без действия, получить 2 Электричества", "1 Силикаты + 1 Металл + 2 Уран", [specific("silicates", 1), specific("metalOre", 1), specific("uranium", 2)], false, { resource: "electricity", qty: 2 }),
  B("hram", "Храм", "industry", "Мистицизм", 1, "Без действия, ни денег, ни ресурсов — сжигает 1 любую выбранную карту действия из руки (обычный сброс на дно колоды, без штрафных эффектов перебора) и даёт +1💰 за каждый город любого игрока, чья религия совпадает с религией владельца Храма (атеист/без религии — сравнивать не с чем, доход 0)", "1 Силикаты + 1 Лес + 2 разных пищевых ресурса", [specific("silicates", 1), specific("wood", 1), category("food", 2)]),
  B("kosmodrom", "Космодром", "industry", "Космонавтика", 6, "Без действия — 1 Углеводороды + 2 Редкоземельные + 2 Металла + 1 Уран, без денег, без лимита цикла — +1 компонент корабля, накопительно за партию; 3 компонента = 🏆 победа через космос", "1 Силикаты + 1 Металл + 2 Редкоземельные", [specific("silicates", 1), specific("metalOre", 1), specific("rareEarth", 2)], true),

  // 🗣 Социальные
  B("upravlenie", "Управление", "social", "Всеобщая воинская повинность", 4, "Без действия, без лимита разом за ход/цикл — 2 функции на выбор: +1 действие в этот ход, или добор 1 карты с колоды; база 5💰 за каждую, цена следующего использования (общая на обе функции) растёт кратно базе: 5, 10, 15, 20...", "1 Силикаты + 1 Лес + 2 разных пищевых ресурса", [specific("silicates", 1), specific("wood", 1), category("food", 2)]),
  B("universitet", "Университет", "social", "Образование", 3, "5💰, без действия — то же открытие технологии, что и карта «Учёный» (та же механика лидерства по веткам), только через здание и с доплатой 5💰 сверху обычной цены исследования", "1 Силикаты + 1 Лес + 2 разных пищевых ресурса", [specific("silicates", 1), specific("wood", 1), category("food", 2)]),
  // Технология «Интернет» переименована в «Геологоразведка» с совсем другим эффектом (по прямому
  // запросу) — здание «Интернет» перекинуто на «Компьютеры» (та же эпоха 6, ветка 0, соседняя ячейка
  // дерева — тематически ближе «Интернету», чем геологоразведка).
  B("internet", "Интернет", "social", "Компьютеры", 6, "5💰, без действия, выбрать другого игрока — сравняться с ним по всем веткам, где он впереди (подтянуть свои технологии до его глубины); если он нигде не впереди — активировать нельзя, деньги не списываются", "1 Силикаты + 1 Металл + 2 Редкоземельные", [specific("silicates", 1), specific("metalOre", 1), specific("rareEarth", 2)]),
