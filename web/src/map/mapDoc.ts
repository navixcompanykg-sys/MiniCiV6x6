...
export interface TileData {
  ...
  volcano?: boolean;
  /** «Истощение ресурсов» (катаклизм ТЗ §15.1, по прямому запросу — «полное исчезновение ресурса с
   * карты наносит очень большой ущерб игровой механике и приводит к каскадному эффекту; вместо этого
   * ресурс помечается перечёркиванием, добыча временно блокирована, снятие блокировки происходит при
   * попытке его добыть») — `resource` НЕ снимается (как раньше), тайл остаётся визуально «тем же»
   * ресурсом, просто перечёркнутым и временно недобываемым. Первая попытка добычи этого типа в
   * регионе снимает блокировку вместо начисления единицы на склад (GameSession.
   * restoreOneBlockedResourceHex); следующая попытка добывает как обычно. Три других катаклизма,
   * тоже убирающих ресурс (Извержение вулкана — `volcano`, Деградация рельефа, Таяние
   * льдов/Затопление — `sinkTileUnderwater`), этот флаг НЕ используют и продолжают уничтожать ресурс
   * насовсем — по прямому уточнению это единственные случаи, где потеря ресурса оправдана самой
   * сменой рельефа (опустынивание/образование вулкана/затопление), а не голым «исчезновением». */
